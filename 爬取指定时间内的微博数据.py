...
设置文件储存的路径 
'''
path = "total_content_comments.csv" #总的微博博文与评论的全部信息
path1 = 'weibo_content.csv'#存取的是微博博文的信息（不包含评论）
path2 = 'weibo_comments.csv' #只保存评论的信息
...
cookie_list = [
    # 手机号
    {'cookie':'_T_WM=34572239173; SCF=AlIs1v-XMv2toNTZyYOwe_daygEHyAFAaIQZbbNFRsnPYPcWMEjNZ4lIhf8b8DOMB4X1SPhg7PVSoUdBgTwIgzg.; SUB=_2A25L4XhDDeRhGeBL6lsU-C_LzjSIHXVon_WLrDV6PUJbktANLU3WkW1NRyj3-JHp8qM1BdP-2dP6JSPI6u5urtzG; SUBP=0033WrSXqPxfM725Ws9jqgMF55529P9D9W5sKw-6c9XRO4anLqZQS3Xu5NHD95QcSK24SKnpS0-RWs4DqcjyCJHydJ4Li--fiKLhi-2Ri--fiK.7iKn0i--4i-2fi-82; ALF=1728877843; MLOGIN=1; WEIBOCN_FROM=1110006030; XSRF-TOKEN=a8fe69; M_WEIBOCN_PARAMS=oid%3D5078472442253373%26lfid%3D5078472442253373%26luicode%3D20000174%26uicode%3D10000011%26fid%3D2304136519581758_-_WEIBO_SECOND_PROFILE_WEIBO'}]

cookie = random.choice(cookie_list)['cookie']

"""
    "_T_WM=34572239173; SCF=AlIs1v-XMv2toNTZyYOwe_daygEHyAFAaIQZbbNFRsnPYPcWMEjNZ4lIhf8b8DOMB4X1SPhg7PVSoUdBgTwIgzg.; SUB=_2A25L4XhDDeRhGeBL6lsU-C_LzjSIHXVon_WLrDV6PUJbktANLU3WkW1NRyj3-JHp8qM1BdP-2dP6JSPI6u5urtzG; SUBP=0033WrSXqPxfM725Ws9jqgMF55529P9D9W5sKw-6c9XRO4anLqZQS3Xu5NHD95QcSK24SKnpS0-RWs4DqcjyCJHydJ4Li--fiKLhi-2Ri--fiK.7iKn0i--4i-2fi-82; ALF=1728877843; MLOGIN=1; WEIBOCN_FROM=1110006030; XSRF-TOKEN=a8fe69; M_WEIBOCN_PARAMS=oid%3D5078472442253373%26lfid%3D5078472442253373%26luicode%3D20000174%26uicode%3D10000011%26fid%3D2304136519581758_-_WEIBO_SECOND_PROFILE_WEIBO"
    "https://m.weibo.cn/search?containerid=100103type%3D1%26q%3D%E5%BB%B6%E8%BF%9F%E9%80%80%E4%BC%91"
"""
...
headers = {
    ...
    'Referer':'https://m.weibo.cn/search?containerid=100103type%3D1%26q%3D%E5%BB%B6%E8%BF%9F%E9%80%80%E4%BC%91',
...
def get_title_id():
    for page in range(1, 50):  # 每个页面大约有9个话题
        ...
        time.sleep(1)
        # 该链接通过抓包获得
        api_url = "https://s.weibo.com/weibo?q={}&typeall=1&suball=1&timescope=custom:{}-{:0>2d}-{:0>2d}-{}:{}-{:0>2d}-{:0>2d}-{}&Refer=g&page={}".format(
            index, year, month1, day1, h1, year, month2, day2, h2, page) ##月得是两位数,3前面要补0变成03
        # print(api_url)

        rep1 = requests.get(url=api_url, headers=headers, verify=certifi.where())
        # rep1.enconding= rep1.apparent_encoding
        try:
            # rep = rep1.text.encode('utf-8').decode('utf8')
            rep=rep1.text # 获取ID值并写入列表comment_ID中
            comment_ID=re.findall(r'(?<=mid=")\d{16}', rep)
...
'''
is_continue='y'
start_date = pd.to_datetime('2024/9/10')
end_date = pd.to_datetime('2024/9/15')
def spider_title(comment_ID):
    ...
    try:
        html_text = requests.get(url=article_url, headers=headers, verify=certifi.where()).text
        # 发布日期
